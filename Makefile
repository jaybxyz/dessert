...
LD_FLAGS = -X github.com/kogisin/dessert/cmd.Version=$(VERSION) \
	-X github.com/kogisin/dessert/cmd.Commit=$(COMMIT)

BUILD_FLAGS := -ldflags '$(LD_FLAGS)'

build: go.sum
ifeq ($(OS),Windows_NT)
	@echo "Building dessert binary..."
	@go build -mod=readonly $(BUILD_FLAGS) -o build/dessert.exe .
else
	@echo "Building dessert binary..."
	@go build -mod=readonly $(BUILD_FLAGS) -o build/dessert .
endif

install: go.sum
	@echo "Installing dessert binary..."
